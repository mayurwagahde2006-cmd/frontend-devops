...
const RepoCard = ({ repo, isImported, onImport }) => {
  ...
  const statusText =
    status === 'success' ? 'Deployed' : status === 'failed' ? 'Failed' : 'Pending';

  const lastDeploy = repo.lastDeploy || '2 hours ago'; 
  return (
    <div className="bg-[var(--card-bg)] rounded-xl p-5 border border-[var(--border-color)] hover:-translate-y-1 transition-all shadow-custom">
      <div className="flex justify-between items-start mb-3">
        ...
        {/* Show status only for imported repos, or if status is meaningful */}
        {isImported && (
      ...
      <div className="flex justify-between items-center">
        <span className="text-xs text-[var(--text-secondary)]">
          <i className="far fa-clock mr-1"></i> Last deploy: {lastDeploy}
        </span>
        {isImported ? (
          <button
            ...
            // Optionally add onClick to navigate to pipeline view
          >
